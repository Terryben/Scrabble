...
def genetic_search(problem, fitness_fn, mutate_fn, ngen=1000, pmut=0.1, n=20):
    ...
    return genetic_algorithm(states[:n], fitness_fn, mutate_fn, ngen, pmut)


def genetic_algorithm(population, fitness_fn, mutate_fn, ngen=1000, pmut=0.1):
    "[Fig. 4.8]"
    for i in range(ngen):
        new_population = []
...
def mate(seq, oseq):
    "Return a new individual crossing seq and oseq."
    c = random.randrange(len(seq))
    return seq.__class__(seq[:c] + oseq[c:])
class GAState:
